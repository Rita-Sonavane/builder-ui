...
import { TextPropertyDescriptor } from "../ElementDescriptions/TextPropertDescriptor";
...
import { GroupElementDesigner } from "../GroupElementManipulation/GroupElementDesigner";
 ...
 */
export class TextDesigner extends ElementDesigner {
    ...
    public newElementInstance(director: HTMLDivElement, x: number, y: number, id_outer: any, id_inner: any): HTMLElement {
        ...
        element.id = "ao-text" + id_outer;
        element.setAttribute("style", "box-sizing: border-box; min-width:10vw;min-height:3vh;  position: absolute;  opacity: 1; height:3vh;top: 570px; left: 397px; width: 10vw;border-width:1px ; border-style:solid; border-color:  #6bc3e1;");
        element.style.left = x - 490 + 'px';
        element.style.top = y - 60 + 'px';

        let textField = document.createElement("div");
        let innerid = textField.id = "Text" + id_inner;
        textField.innerText = "....editme.....";
        textField.setAttribute("style", "min-height: -webkit-fill-available;box-sizing: border-box; position: relative;padding: 0px; cursor: inherit; background-color: #fff; border: none; font-family: var(--font_default); font-size: 14px; font-weight: 700; color: #000; letter-spacing: 2px; line-height: 1; border-radius: 5px;");
    ...
    <div class="resize se" id="se" draggable="false" contenteditable="false"></div>`;
        ...
        var groupElementSelection = new GroupElementSelection(selectedElement, _outer);
    ...
    updateProperties(selectedElement: any) {
        ...
        let propertyDescriptor = new TextPropertyDescriptor();
        let propertyEditor = new PropertyEditor();
        let designerConversion = new DesignerConversions;

        console.log("unit with height", selectedElement.style.height);
